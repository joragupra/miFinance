...
public class BalanceServiceTest {
    ...
    public void testAddEntry() {
      ...
      service.addEntry(balanceId, entryDescription, entryDate, entryAmountCents);
      ...
      assertEquals(entryAmountCents, balance.entries().get(0).amount().cents());
    }
    ...
    public void testUpdateEntry() {
      ...
      final String entryGuid = service.retrieveBalance(balanceId).entries().get(0).guid();

      service.updateEntry(balanceId, entryGuid, modifiedEntryDescription, new Date(), modifiedAmountCents);

      Balance balance = service.retrieveBalance(balanceId);

      assertEquals(1, balance.entries().size());
      assertEquals(modifiedEntryDescription, balance.entries().get(0).description());
      assertEquals(modifiedAmountCents, balance.entries().get(0).amount().cents());
    }
